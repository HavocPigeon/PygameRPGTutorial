...
class UI:
    ...
    def show_exp(self, exp):
        text_surf = self.font.render(str(f'exp:{int(exp)}'), False, TEXT_COLOUR)
        x = WIDTH - 10
        y = HEIGHT - 10
        text_rect = text_surf.get_rect(bottomright = (x, y))

        pygame.draw.rect(self.display_surface, UI_BG_COLOUR, text_rect.inflate(10, 0))
        self.display_surface.blit(text_surf, text_rect)
        pygame.draw.rect(self.display_surface, UI_BORDER_COLOUR, text_rect.inflate(10, 0), 3)
    ...
    def magic_overlay(self, magic_index, has_switched):
        bg_rect = self.selection_box((ITEM_BOX_SIZE + 15), HEIGHT - (ITEM_BOX_SIZE + 10), has_switched)
